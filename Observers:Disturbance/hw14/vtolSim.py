...
while t < P.t_end:  
   ...
   #inner loop
    while t < t_next_plot:  
        ...
        n_z = np.array([[noise_z.random(t)], [noise_th.random(t)]]) #noise.random(t)  # simulate sensor noise
        n_h = np.array([noise_h.random(t)])
        ...
        vtol.F_lon , x2hat, d1hat = lon_controller.update_lon(href,x_h + n_h + d) #vtol.state.item(2),vtol.state.item(5))
        vtol.t_lat_theta , x1hat, d2hat = lat_controller.update_lat(zref,x_z + n_z + d)
    ...
    dataPlotObserver.update(t, vtol.state, x1hat,x2hat, d, d1hat, d2hat)
